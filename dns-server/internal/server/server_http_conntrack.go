...
type conntrackBucketsResponse struct {
	...
	Buckets           []conntrackBucket   `json:"buckets"`
}

type conntrackBucket struct {
...
func (s *HTTPServer) handleListConntrackBuckets(w http.ResponseWriter, req *http.Request) (int, error) {
	...
	buckets, err := util.Seq2ToSlice(int(1+uint(tr.End-tr.Start+1)/interval), bucketSeq)
	if err != nil {
		return http.StatusInternalServerError, err
	}

	resp.Buckets = mapConntrackBuckets(buckets)
...
func mapConntrackBuckets(buckets []conntrack.Bucket) []conntrackBucket {
	res := make([]conntrackBucket, len(buckets))
	for i, bucket := range buckets {
		res[i] = mapConntrackBucket(bucket)
	}
	return res
}

func mapConntrackBucket(bucket conntrack.Bucket) conntrackBucket {
	entries := make([][9]uint64, len(bucket.Entries))
	for j, entry := range bucket.Entries {
		entries[j] = [9]uint64{
			uint64(entry.Protocol),
			uint64(entry.SrcIP.Uint32()),
			uint64(entry.DstIP.Uint32()),
			uint64(entry.DstPort),
			uint64(entry.BytesOrig),
			uint64(entry.BytesReply),
			uint64(entry.PacketsOrig),
			uint64(entry.PacketsReply),
			uint64(len(entry.ConnIDs)),
		}
	}
	return conntrackBucket{
