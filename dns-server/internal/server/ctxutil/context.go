...
func GetDNSQueryID(ctx context.Context) types.QueryID {
	id, _ := ctx.Value(contextKeyDNSQueryID{}).(types.QueryID)
	return id
}
...
func GetDNSQueryClientIP(ctx context.Context) types.IPv4 {
	ip, _ := ctx.Value(contextKeyDNSQueryClientIP{}).(types.IPv4)
	return ip
}
