...
export type Result<T> =
	...
	| { value?: undefined; error: Error | string };
...
export function listenStream<T>(
	stream: Stream<T>,
	listener: (res: Result<T> | 'cancelled') => Promise<void>
): void {
	setTimeout(async () => {
		for (;;) {
			try {
...
export function websocketStream<T>(
	provider: () => WebSocket,
...
export function mapStream<T, R>(
	stream: Stream<T>,
	mapper: (value: Result<T>, sink: (value: R) => void) => void
): Stream<R> {
		...
		next(): Promise<T> {
			...
			return new Promise<T>((resolve, reject) => {
				...
				promise
					.then(resolve, reject)
					.catch(() => {})
					.then(() => rejectCallbacks.delete(reject));
			});
		},
		cancel() {
			if (!cancelled) {
